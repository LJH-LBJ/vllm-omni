...
def compute_and_log_stage_request_stats(stats: "StageRequestStats") -> None:
    ...
    log_stage_request_stats(
        stats.stage_id,
        stats.request_id,
        int(stats.batch_size or 0),
        int(stats.num_tokens_out),
        float(stats.stage_gen_time_ms),
        float(tokens_per_s),
        int(stats.rx_transfer_bytes),
        float(stats.rx_decode_time_ms),
        float(rx_mbps),
    )
...
class StageRequestStats:
    ...
    rx_in_flight_time_ms: float = 0.0
...
class RunSummary:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "e2e_total_time_ms": self.e2e_total_time_ms,
            "e2e_sum_time_ms": self.e2e_sum_time_ms,
...
def record_stage_metrics(
    ...
    stage_total_tokens[stage_id] += int(stats.num_tokens_out)
    ...
    if stats.stage_stats is not None:
        stage_data["stage_stats"] = {
            "total_token": int(stats.stage_stats.total_token),
            "total_gen_time": float(stats.stage_stats.total_gen_time),
        }
    # Only record num_tokens_in for stage 0 (initial prompt)
    if stage_id == 0:
        stage_data["num_tokens_in"] = int(stats.num_tokens_in)
        stage_total_tokens[stage_id] += int(stats.num_tokens_in)
...
class OrchestratorAggregator:
    """Aggregator for orchestrator-level metrics.

    Acts as the single interface for recording per-stage request stats,
    transfer edge stats, and end-to-end request completions. Downstream
    loggers (text or Prometheus) can subscribe via the logger manager.
    """
    ...
    def __init__(
        ...
        self.wall_start_ts: float = float(wall_start_ts if wall_start_ts is not None else time.time())
        self.last_finish_ts: float = float(self.wall_start_ts)
    ...
    def set_logger_manager(self, manager: Any) -> None:
        self.logger_manager = manager

    def _maybe_log_interval(self) -> None:
        if self.logger_manager is not None:
            try:
                self.logger_manager.maybe_log()
            except Exception:
                logger.debug("Failed to emit periodic stats", exc_info=True)

    def _as_stage_request_stats(
        self,
        stage_id: int,
        req_id: Any,
        metrics: StageRequestStats | dict[str, Any],
    ) -> StageRequestStats:
        if isinstance(metrics, StageRequestStats):
            ...
            return stats
        m = metrics or {}
        stage_stats_raw = m.get("stage_stats", None)
        stage_stats = None
        if stage_stats_raw is not None:
            try:
                stage_stats = StageStats(
                    total_token=int(stage_stats_raw.get("total_token", 0)),
                    total_gen_time=float(stage_stats_raw.get("total_gen_time", 0.0)),
                )
            except Exception:
                stage_stats = None
        return StageRequestStats(
            stage_id=stage_id,
            request_id=req_id,
            num_tokens_in=int(m.get("num_tokens_in", 0) or 0),
            num_tokens_out=int(m.get("num_tokens_out", 0) or 0),
            stage_gen_time_ms=float(m.get("stage_gen_time_ms", 0.0) or 0.0),
            batch_id=m.get("batch_id"),
            batch_size=m.get("batch_size"),
            rx_decode_time_ms=float(m.get("rx_decode_time_ms", 0.0) or 0.0),
            rx_transfer_bytes=int(m.get("rx_transfer_bytes", 0) or 0),
            rx_in_flight_time_ms=float(m.get("rx_in_flight_time_ms", 0.0) or 0.0),
    ...
    def on_stage_metrics(self, stage_id: int, req_id: Any, metrics: StageRequestStats | dict[str, Any]) -> None:
        ...
        if self.enable_debug_events:
            compute_and_log_stage_request_stats(stats)
        ...
        try:
            batch_id_raw = stats.batch_id
            if batch_id_raw is not None:
                batch_id = int(batch_id_raw)
                if batch_id not in self.stage_seen_batches[stage_id]:
                    self.stage_total_time_ms[stage_id] += float(stats.stage_gen_time_ms)
                    self.stage_seen_batches[stage_id].add(batch_id)
        except Exception:
            pass
        rx_b = float(stats.rx_transfer_bytes)
        rx_ms = float(stats.rx_decode_time_ms)
        in_flight_ms = float(stats.rx_in_flight_time_ms)
        ...
        if self.enable_debug_events and stage_id > 0:
            log_transfer_rx(
                stage_id - 1,
                stage_id,
                req_id,
            ...
            if combined is not None:
                ...
                tx_event = TransferEdgeStats(
                    ...
                    rx_decode_time_ms=float(rx_ms),
                    in_flight_time_ms=float(in_flight_ms),
                )
    ...
    def on_finalize_request(
        ...
        self.request_events.append(per_req_record)
        if self.enable_debug_events:
            logger.info(
                pformat(
                    {
    ...
    def build_run_summary(self, final_stage_id_to_prompt: dict[str, int] | int | None = None) -> RunSummary:
        ...
        return RunSummary(
            ...
            final_stage_id=self.final_stage_map,
...
StageRequestMetrics = StageRequestStats
