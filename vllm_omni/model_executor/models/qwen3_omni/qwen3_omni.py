...
class Qwen3OmniMoeForConditionalGeneration(
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        if self.model_stage == "thinker":
            ...
            # Model-instance cache for thinker decode embeddings, keyed by
            # request_id.  Bypasses model_intermediate_buffer to avoid
            # buffer persistence issues with the replay mechanism.
            self._decode_embed_cache: dict[str, torch.Tensor] = {}
    ...
    def talker_preprocess_prefill(self, input_ids: torch.Tensor, input_embeds: torch.Tensor, **info_dict: dict):
        ...
        if thinker_sequences is None:
            thinker_sequences = ids_chatml
        # full_total includes decode tokens already generated by the thinker (e.g. 101).
        # Used only for the prefill_exhausted sentinel jump when bootstrap completes.
        ...
        # embed_available is the number of prompt-side embeddings delivered so far.
        # thinker_sequences may contain more tokens (decode tokens beyond the prompt)
        # for which no embeddings exist in thinker_sequence_embeds.  Never process
        # beyond embed_available — the assistant <|im_start|>assistant\n section is
        # part of the prompt and arrives naturally as embed_available grows.
        embed_available = min(
            thinker_sequence_embeds.shape[0],
            thinker_hidden_states.shape[0],
        )
        # Resolve decode embed so _thinker_to_talker_prefill can use it for bootstrap.
        ...
        remaining_thinker_tokens = max(embed_available - chunk_offset, 0)
