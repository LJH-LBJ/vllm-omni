...
class Qwen3OmniMoeForConditionalGeneration(
    ...
    def talker_preprocess_prefill(self, input_ids: torch.Tensor, input_embeds: torch.Tensor, payload: OmniPayload):
        ...
        logger.info(f"[DBG_READ] reading num_processed_thinker_tokens={num_processed_thinker_tokens} embed.prefill.shape={thinker_sequence_embeds.shape}", flush=True)
        ...
        if isinstance(decode_assistant_fill, torch.Tensor) and decode_assistant_fill.numel() == 0:
        ...
        _next_nptt = chunk_offset + len(thinker_sequences_chunk)
        logger.info(f"[DBG_WRITE] req={request_id[-16:] if request_id else '?'} writing num_processed_thinker_tokens={_next_nptt} (offset={chunk_offset}+len={len(thinker_sequences_chunk)})", flush=True)
        update_dict.setdefault("meta", {})["num_processed_thinker_tokens"] = _next_nptt
