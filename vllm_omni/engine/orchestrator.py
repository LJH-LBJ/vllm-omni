...
class Orchestrator:
                ...
                if stage_client.stage_type == "diffusion":
                    ...
                    if output is not None:
                        ...
                        if req_state is not None:
                            if getattr(output, "error", None) is not None:
                                parent_id = self._cfg_tracker._companion_to_parent.get(output.request_id, output.request_id)
                                await self.output_async_queue.put(
                                    {
                                        "type": "error",
    ...
    async def _prewarm_async_chunk_stages(
        ...
        # Pre-arm stage-1+ with full placeholder prompt IDs.
        # Embedding-count gating (preventing garbage KV) is handled in
        # chunk_transfer_adapter._poll_single_request which caps
        # request.num_prompt_tokens to the cumulative available embeddings.
        try:
